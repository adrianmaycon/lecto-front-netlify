            ...
            categorie.attributes.categories.data.map((subCategoria) => {
                ...
                subCategoria.attributes.parents.data.map((categoria) => {
                    if (!categoriesElements.categoriesName.includes(categoria.attributes.name)) {
                        categoriesElements.categoriesName.push({ name: categoria.attributes.name, slug: categoria.attributes.slug }); // Salvando o nome das Categorias no objeto      -> Alimentação e Vida e Saúde
                    }
                })
                        ...
                        <div className="conteudo">
                            ...
                            <div className="responsive">
                                <div className='container-recomendamos'>
                                    ...
                                    {categoriesElements.categoriesName.map((categorias) => {
                                        /* Condicional para exibir apenas as listagens da categoria que está no URL
                                        Exemplo: Ao acessar /alimentacao irá exibir apenas as subcategorias da alimentação */
                                        if (categorias.slug == window.location.href.split(window.location.origin)[1].split('/')[1] && contCategorys <= 0) {
                                            contCategorys += 1;
                                            return (
                                                <div>
                                                    {/* Propriedades para a Categoria começar com letra maiúscula */}
                                                    <h2>{categorias.name.charAt(0).toUpperCase() + categorias.name.slice(1)}</h2>  { /* Retorna o nome da categoria, por ex: Alimentação e Vida e Saúde com a 1ª letra maiúscula */}
                                                    ...
                                                    Caso a categoria não tenha sido add nessa variável, ela adiciona e exibe o conteúdo*/
                                                        if (subCategorias.attributes.categories.data[0].attributes.parents.data[0].attributes.name == categorias.name && !itensSubCategorias.includes(subCategorias.attributes.categories.data[0].attributes.name)) {
